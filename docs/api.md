...
- 创建、编辑和接管都会校验最终的 `owner-vmname`；仅允许字母、数字、连字符和点，点分隔的每一段必须以字母或数字开头和结尾。
- `disk_gb` 至少 20。
...
worker 对任务初次失败最多自动重试 5 次，重试间隔依次为 1、2、4、8、16 分钟。达到上限后保持失败状态，仍可通过本接口人工重试。
...
- VM 创建和普通用户编辑不会使用这里返回的全量 security group。
- 管理员从 `Admin / All VMs` 编辑或接管 VM 时，会使用这里的全量结果，但前端按目标 owner 过滤展示，后端也按目标 owner 校验。
